...
TempAbove10Distinct = TableReadings.select(["year","month","station"]).filter((TableReadings['year'] <=2014) & (TableReadings["year"]>=1950)
& (TableReadings["Temp"] > 10)).groupby(["year","month"]).agg(F.countDistinct("station").alias("station_count")).orderBy("station_count",ascending=False)
